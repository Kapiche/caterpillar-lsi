...
class LSIModel(object):
    ...
    def compare_document(self, term_frequencies, filter_docs=None):
        ...
        if len(term_frequencies) != self.num_terms:
            raise LSIModel.BadDocumentException(
                "Document vector length ({}) is not equal to num terms ({})"
    ...
    def classify_document(self, term_frequencies):
        """Return the document-feature vector for the document represented by ``term_frequencies``."""
        if len(term_frequencies) != self.num_terms:
            raise LSIModel.BadDocumentException(
                "Document vector length ({}) is not equal to num terms ({})"
                .format(len(term_frequencies), self.num_terms)
        ...
        # Compute document-feature vector
        return self._compute_document_feature_vector(term_frequencies)
...
class LSIPlugin(AnalyticsPlugin):
    ...
    def compare_index_using_model(self, reader):
        """
        Compare all the documents in ``index`` with each other using the model generated by the running of this plugin
        for feature extraction. A document is considered to be a frame on the index.
        ...
        if self._model is None:
            raise RuntimeError('Cannot compare index using model before the plugin has been run.')

        # Calculate document-feature vectors for each doc using model
        ...
        for i, term_freqs in enumerate(D.T):
            df_vectors[order[i]] = self._model.classify_document(term_freqs)

        # Calculate similarities between each document
        return {
            f_id: {
                other_id: 1 - spatial.distance.cosine(df_vectors[f_id], df_vectors[other_id])
                for other_id in order
            } for f_id in order
        }

    def compare_index_with_model(self, reader, model_filter_query=None):
        """
    ...
    def _build_covariance_matrix(index, row_terms=None):
        """
        Build a covariance matrix (term x document frequency) for the specified index. If ``row_terms`` is specified,
